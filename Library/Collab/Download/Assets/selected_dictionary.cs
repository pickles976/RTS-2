...
public class selected_dictionary : MonoBehaviour
{
    ...
    public void addSelected(GameObject go)
    {
        ...
        if (!(selectedTable.ContainsKey(id)))
        {
            ...
            //selecting buildings
            if (!(go.GetComponent<structure_script>() == null))
            {
                selectedTable.Add(id, go); //add to the table
                go.GetComponent<structure_script>().createUI(); //instantiate the build UI
            }
        }
    ...
    public void deselectAll()
    {
        ...
        foreach(KeyValuePair<int,GameObject> pair in selectedTable)
        {
            if (pair.Value != null)
            {
                ...
                //removing buildings
                if (!(selectedTable[pair.Key].GetComponent<structure_script>() == null))
                {
                    selectedTable[pair.Key].GetComponent<structure_script>().destroyUI(); //instantiate the build UI
                }
